...
struct graphdef {
  ...
  integer gdatasize, rdatasize, nvwgtsize, labelsize, freegraph;
};
...
* This data structure holds various working space data
**************************************************************************/
struct graphspacedef {
  idxtype *core;			
  integer maxcore, ccore;
};

typedef struct graphspacedef GraphSpaceType;
...
struct controldef {
  ...
  GraphSpaceType gspace;	/* Graph Space Informations */
  ...
  integer lastvtx;
  double vfactor, efactor;      /* see defs.h: ONMETIS_VFACTOR */

  integer depth;                    /* recursion depth */
  integer nproc;                    /* number of cpu's for work */
  integer ddist_width, ddist_start;
  integer mmdswitch;

  /* wallclock timers */
  double timerright, timerleft, timersubgraphs, timerfirstpart;
  double timernodebisection, timersplitgraphorder;
};
